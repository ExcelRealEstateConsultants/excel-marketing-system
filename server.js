...
function saveGmailToken(tokens) {
  fs.writeFileSync(GMAIL_TOKEN_FILE, JSON.stringify(tokens, null, 2));
}
...
app.get("/auth/google", (req, res) => {
  if (!GOOGLE_CLIENT_ID || !GOOGLE_CLIENT_SECRET || !GOOGLE_REDIRECT_URI) {
    return res
      .status(500)
      .send(
        "Google OAuth is not configured. Please check GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, and GOOGLE_REDIRECT_URI.",
      );
  }
...
const SCHEDULED_CAMPAIGN_FILE = path.join(
  __dirname,
  "scheduled-campaigns.json",
);
...
function readJsonFile(filePath, fallback) {
  ...
  try {
    return JSON.parse(
      fs.readFileSync(filePath, "utf8") || JSON.stringify(fallback),
    );
  } catch (error) {
    return fallback;
  }
...
function normalizeStageForPipeline(value) {
  ...
  const matched = PIPELINE_STAGES.find(
    (stage) => String(stage).toLowerCase() === raw,
  );
...
function normalizeContactPipelineFields(contact = {}) {
  const stage = normalizeStageForPipeline(
    contact.stage || contact.pipelineStage || contact.status || "New Lead",
  );
  ...
  return {
    ...
    stage,
    stageUpdatedAt:
      contact.stageUpdatedAt || contact.updatedAt || contact.createdAt || now,
    stageHistory:
...
function normalizeContactsForPipeline(contacts) {
  return (Array.isArray(contacts) ? contacts : []).map(
    normalizeContactPipelineFields,
  );
}
...
function isOverdueTask(task) {
  return (
    ...
    !task.done &&
    task.due &&
    String(task.due).slice(0, 10) < todayDateString()
  );
}
...
function getActiveEmailProvider(providerData) {
  if (providerData.activeProvider === "microsoft" && providerData.microsoft)
    return "microsoft";
  if (providerData.activeProvider === "gmail" && providerData.gmail)
    return "gmail";
...
function buildContactFromPayload(payload = {}) {
  const stage = normalizeStageForPipeline(
    payload.stage || payload.pipelineStage || payload.status || "New Lead",
  );
  const notes = [];
...
app.post("/api/email-images", (req, res) => {
  ...
  if (!image.data || !String(image.data).startsWith("data:image/")) {
    return res
      .status(400)
      .json({ success: false, error: "Valid image data is required." });
  }
...
async function sendTwilioSms({ to, body }) {
  if (!TWILIO_ACCOUNT_SID || !TWILIO_AUTH_TOKEN || !TWILIO_PHONE_NUMBER) {
    throw new Error(
      "Twilio is not configured. Please check TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, and TWILIO_PHONE_NUMBER in your environment variables.",
    );
  }
  ...
  if (!response.ok) {
    throw new Error(
      result.message || `Twilio SMS failed for ${maskPhone(formattedTo)}`,
    );
  }
...
app.get("/auth/google/callback", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send(
        `Google connection failed: ${escapeHtml(error.message || "Unknown error")}`,
      );
  }
});
...
app.get("/api/contacts/export", (req, res) => {
  ...
  const rows = contacts.map((contact) => [
    ...
    Array.isArray(contact.tasks)
      ? contact.tasks.filter((task) => !task.done).length
      : 0,
    Array.isArray(contact.notes)
      ? contact.notes.map((note) => note.text || "").join(" | ")
      : "",
  ]);
  ...
  res.setHeader(
    "Content-Disposition",
    'attachment; filename="excel-crm-contacts-export.csv"',
  );
  res.send(csv);
});
...
app.put("/api/contacts/:id", (req, res) => {
  ...
  contacts[i] = {
    ...
    spouseEmailAlt: req.body.spouseEmailAlt || "",
    ...
    stageUpdatedAt: stageChanged
      ...
      : contacts[i].stageUpdatedAt || new Date().toISOString(),

    stageHistory: stageChanged
      ? [
          ...(Array.isArray(contacts[i].stageHistory)
            ? contacts[i].stageHistory
            : []),
...
app.put("/api/contacts/bulk-update", (req, res) => {
  ...
  const addTags = Array.isArray(req.body.addTags)
    ? req.body.addTags.map((t) => String(t).trim()).filter(Boolean)
    : [];
...
app.delete("/api/contacts/bulk-delete", (req, res) => {
  ...
  const remaining = contacts.filter(
    (contact) => !ids.includes(String(contact.id)),
  );
...
app.get("/api/contacts/duplicates", (req, res) => {
  ...
  Object.entries(emailMap).forEach(([value, items]) => {
    if (items.length > 1)
      duplicates.push({ type: "email", value, contacts: items });
  });

  Object.entries(phoneMap).forEach(([value, items]) => {
    if (items.length > 1)
      duplicates.push({ type: "phone", value, contacts: items });
  });
...
app.get("/api/tasks", (req, res) => {
  ...
  tasks.sort(
    (a, b) => new Date(a.due || "9999-12-31") - new Date(b.due || "9999-12-31"),
  );
...
app.get("/api/sms-activity", (req, res) => {
  ...
  if (contactId) {
    filtered = filtered.filter(
      (item) => String(item.contactId || "") === contactId,
    );
  }

  if (phone) {
    filtered = filtered.filter(
      (item) => normalizeSmsPhone(item.to || item.from || "") === phone,
    );
  }
...
app.post("/api/send-sms", async (req, res) => {
  ...
  if (!to) {
    return res.status(400).json({
      success: false,
      error: "A phone number is required before sending a text.",
    });
  }
  ...
  if (!message) {
    return res.status(400).json({
      success: false,
      error: "A message is required before sending a text.",
    });
  }
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: failedRecord.error, sms: failedRecord });
  }
});
...
app.post("/api/send-text-campaign", async (req, res) => {
  ...
  if (!message) {
    return res.status(400).json({
      success: false,
      error: "A message is required before sending a text campaign.",
    });
  }
  ...
  if (recipients.length === 0) {
    return res.status(400).json({
      success: false,
      error:
        "At least one recipient is required before sending a text campaign.",
    });
  }
...
app.get("/api/dashboard-stats", (req, res) => {
  ...
  const totalClicks = activity.reduce(
...
app.post("/api/scheduled-campaigns", (req, res) => {
  ...
  if (!req.body.scheduledAt) {
    return res
      .status(400)
      .json({ success: false, error: "Scheduled date/time is required." });
  }

  if (recipients.length === 0) {
    return res
      .status(400)
      .json({ success: false, error: "At least one recipient is required." });
  }

  if (!req.body.subject || !req.body.html) {
    return res
      .status(400)
      .json({ success: false, error: "Subject and content are required." });
  }
  ...
  if (Number.isNaN(scheduledDate.getTime())) {
    return res
      .status(400)
      .json({ success: false, error: "Scheduled date/time is invalid." });
  }
...
app.delete("/api/scheduled-campaigns/:id", (req, res) => {
  ...
  const existingScheduledCampaign = scheduledCampaigns.find(
  ...
  if (!existingScheduledCampaign) {
    return res
      .status(404)
      .json({ success: false, error: "Scheduled campaign not found" });
  }

  if (existingScheduledCampaign.status === "Sent") {
    return res.status(400).json({
      success: false,
      error: "Sent scheduled campaigns cannot be deleted.",
    });
  }
  ...
  try {
    if (!BREVO_API_KEY || !BREVO_SENDER_EMAIL) {
      return res.status(500).json({
        success: false,
        error:
          "Brevo is not configured. Please check BREVO_API_KEY and BREVO_SENDER_EMAIL in your environment variables.",
      });
    }
    ...
    if (!message) {
      return res
        .status(400)
        .json({ success: false, error: "Email message is required." });
    }
...
async function sendCampaignCore({
  ...
  const unsubscribedEmails = new Set(
    contacts.filter((c) => c.unsubscribed).map((c) => normalizeEmail(c.email)),
  );

  const bouncedEmails = new Set(
    contacts.filter((c) => c.bounced).map((c) => normalizeEmail(c.email)),
  );
  ...
  const allowedRecipients = cleanRecipients.filter(
    (email) => !unsubscribedEmails.has(email) && !bouncedEmails.has(email),
  );

  const skippedRecipients = cleanRecipients.filter(
    (email) => unsubscribedEmails.has(email) || bouncedEmails.has(email),
  );
  ...
  for (const email of allowedRecipients) {
    const pixel = `<img src="${BASE_URL}/track-open?email=${encodeURIComponent(email)}&campaign=${encodeURIComponent(campaignName)}" width="1" height="1" style="display:none;" />`;
...
app.post("/api/send-campaign", async (req, res) => {
  try {
    ...
    const statusCode = error.message.includes("Brevo is not configured")
      ? 500
      : 400;
...
app.post("/api/ai/universal-document", async (req, res) => {
  try {
    ...
    const txn =
      req.body && typeof req.body.txn === "object" ? req.body.txn : {};

    const doc =
      req.body && typeof req.body.doc === "object" ? req.body.doc : {};
    ...
    if (!documentText) {
      return res.status(400).json({
        success: false,
        error: "Readable document text is required.",
      });
    }

    console.log("SERVER: starting universal document analysis");

    console.log("SERVER CHECK", typeof aiAnalyzeUniversalDocument);

    const analysis = await aiAnalyzeUniversalDocument(txn, doc);
    ...
    console.error("Universal document analysis failed:", {
      name: error?.name,
    ...
    return res.status(500).json({
      success: false,
      error: error?.message || "Universal document analysis failed.",
    });
  }
});
